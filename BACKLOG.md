...
- 2026-08-30 — Repaired the production-wide visualization 404 failure and completed Bronze Age insight 02, **The palace was a switchboard, not the economy**. All client-fetched maps, timelines, charts, and evidence registries across Rome, Uruk, Cradles, and the Bronze Age now bypass cached error responses and retry six times with bounded backoff before showing a manual error state. Expanded the production release smoke test from five Uruk CSVs to all twenty-seven live visualization assets, including JSON validity and HTML-404 rejection. The new palace comparison adds five source-keyed institutional circuits—Mari, Hattusha, Knossos, Pylos, and Ugarit—while preserving incomparable archive, grain, flock, and metal-allotment evidence as selective windows rather than a centralization score. Verified 48 data files, 135 sources, 22 datasets, 17 claims, lint, production build, all five Cradles arguments, every Rome/Uruk/Bronze chart with zero loading errors, and 390 × 844 layouts on all four pages. A grid min-content leak on Rome was also fixed; all pages now have zero horizontal overflow at the mobile release viewport.
