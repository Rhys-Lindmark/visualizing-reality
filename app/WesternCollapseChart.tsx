  ...
  return <div className="collapse-chart">
    {loadError?<p className="fiscal-error">{loadError}</p>:<>
      ...
      <section className="collapse-sequence" aria-labelledby="collapse-sequence-title">
        ...
        {selected&&<article className="event-detail"><div style={{background:eventColors[selected.event_type]}}><span>{selected.display_year}</span><b>{selected.event_type}</b></div><div><h6>{selected.event}</h6><p>{selected.what_changed}</p><dl><div><dt>Mechanism</dt><dd>{selected.mechanism}</dd></div></dl><small>{selected.notes}</small></div></article>}
      ...
      <section className="africa-model" aria-labelledby="africa-model-title">
        ...
        <div className="equivalent-bars">{equivalents.map((row,index)=>{const value=selectedValue(row);return <article key={row.region_group}><div><span>{row.region_group}</span><small>{row.loss_status}</small></div><div className="equivalent-bar"><i style={{width:`${(value/maximum)*100}%`,background:index?'#bd1f2e':'#d6a34a'}} /></div><b>{metric==='cavalry'&&row.cavalry_relation==='lower_bound'?'more than ':''}{value.toLocaleString()}</b></article>;})}</div>
      ...
      <section className="eighth-survives" aria-labelledby="eighth-title"><div><span>21 June 445</span><h5 id="eighth-title">Even what remained in Africa was badly damaged</h5><p>Valentinian III’s Novel XIII withdrew seven parts of the former assessment for Numidia and Mauretania Sitifensis after public disaster and attacks.</p></div><div className="eighth-grid" role="img" aria-label="One of eight parts of the prior assessment remained">{Array.from({length:8},(_,index)=><i key={index} className={index===0?'remaining':''}><span>{index===0?'remaining':'remitted'}</span></i>)}</div></section>
    </>}
  </div>;
}
