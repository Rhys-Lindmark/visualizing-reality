...
export default function CradlesEvidenceClocks(){
  ...
  return <div className="cradle-clock-chart">
    ...
    <section className="cradle-clock-comparison" aria-label="Six regional city state and notation sequences"><header><span>Six different sequences</span><h5>The order changed from region to region</h5></header><div>{regions.map(region=><button type="button" key={region.region_slug} className={region.region_slug===selectedRegion?'active':''} onClick={()=>chooseRegion(region.region_slug)}><b>{region.region}</b>{clockOrder.map(clock=>{const row=rows.find(item=>item.region_slug===region.region_slug&&item.clock===clock);return <span key={clock}><i style={{background:clockColors[clock]}}/><small>{clockLabels[clock]}</small><strong>{row?.evidence_status==='evidence_gap'?'Undated':row?.display_date}</strong></span>})}</button>)}</div></section>
  </div>;
}
