...
export default function CradlesCoordinationRoutes(){
  ...
  if(loadState!=='ready'||!selected)return <div className={`data-state cradle-coordination-data-state ${loadState}`} role={loadState==='error'?'alert':'status'}><b>{loadState==='error'?'The coordination routes did not load':'Loading institutional routes…'}</b><span>{loadState==='error'?error:'Comparing inputs, institutions, and outcomes'}</span>{loadState==='error'&&<button type="button" onClick={retry}>Retry routes</button>}</div>;
  ...
  return <div className="coordination-chart" style={{'--region-color':regionColors[selectedRegion]} as React.CSSProperties}>
    ...
    <section className="coordination-portfolio" aria-label={`${selected.region} coordination portfolio`}>
      <div className="coordination-heading"><div><span>{selected.region}</span><h5>Institutions converted resources into coordinated action</h5></div></div>
    ...
    <section className="coordination-readout" aria-live="polite"><div className="coordination-readout-title"><span>{selected.region} · {selected.route_label}</span><h5>{selected.coordinator}</h5><b>{selected.place} · {selected.time_window}</b></div><div className="coordination-readout-body"><p>{selected.observation}</p><strong>{selected.interpretation}</strong></div></section>
    <section className="coordination-overview"><div><span>Six institutional portfolios</span><h5>Different institutions solved different coordination problems</h5></div><div className="coordination-mini-grid">{regions.map(region=><button type="button" key={region.slug} className={region.slug===selectedRegion?'active':''} aria-pressed={region.slug===selectedRegion} onClick={()=>chooseRegion(region.slug)} style={{'--tab-color':regionColors[region.slug]} as React.CSSProperties}><span><i/>{region.name}</span>{rows.filter(row=>row.region_slug===region.slug).map(row=><small key={row.route_slug}>{row.route_label}</small>)}</button>)}</div></section>
  </div>;
}
