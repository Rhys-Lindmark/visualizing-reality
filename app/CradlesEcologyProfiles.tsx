...
export default function CradlesEcologyProfiles(){
  ...
  if(loadState!=='ready'||!selected)return <div className={`data-state cradle-ecology-data-state ${loadState}`} role={loadState==='error'?'alert':'status'}><b>{loadState==='error'?'The ecology profiles did not load':'Loading ecological profiles…'}</b><span>{loadState==='error'?error:'Comparing water, rainfall, crops, transport, and settlement'}</span>{loadState==='error'&&<button type="button" onClick={retry}>Retry comparison</button>}</div>;
  ...
  return <div className="cradle-ecology-chart">
    ...
    <section className="ecology-fingerprint" aria-label={`${selected.region} ecological fingerprint`}><div className="ecology-fingerprint-heading"><div><span>{selected.region}</span><h5>Water, food, movement, and settlement</h5></div></div><div className="ecology-cards">{regionalRows.map(row=><button type="button" key={row.dimension} className={row.dimension===selectedDimension?'active':''} aria-pressed={row.dimension===selectedDimension} onClick={()=>setSelectedDimension(row.dimension)}><i>{dimensionIcons[row.dimension]}</i><span>{dimensionLabels[row.dimension]}</span><b>{row.headline}</b></button>)}</div></section>
    <section className="ecology-readout" aria-live="polite"><div className="ecology-readout-title"><span>{selected.region} · {selected.dimension_label}</span><h5>{selected.headline}</h5><b>{selected.place} · {selected.time_window}</b></div><div className="ecology-readout-body"><p>{selected.observation}</p><strong>{selected.interpretation}</strong></div></section>
