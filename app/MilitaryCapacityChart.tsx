...
export default function MilitaryCapacityChart(){
  ...
  useEffect(()=>{const refresh=(event:PageTransitionEvent)=>{if(event.persisted){setData([]);setLoadError('');setLoadState('loading');setLoadAttempt(value=>value+1);}};window.addEventListener('pageshow',refresh);return()=>window.removeEventListener('pageshow',refresh);},[]);
