...
const colors:Record<string,string>={
  'Rome':'#bd1f2e','Achaemenid Empire':'#8f68a0','Macedonian Empire':'#397f9a','Gallic coalition':'#6e9657','Carthage':'#bf8730','Ptolemaic Kingdom':'#268c87','Seleucid Empire':'#d06a36','Parthian Empire':'#745395','Sasanian Empire':'#53376f',
};
...
export default function MilitaryCapacityChart(){
  ...
  useEffect(()=>{const ctx=canvas.current?.getContext('2d');if(!ctx||!data.length)return;const w=1050,h=520,left=76,right=34,top=50,bottom=66,plotW=w-left-right,plotH=h-top-bottom;const ymax=metric==='iron'?4000:550;const x=(year:number)=>left+((year+500)/1000)*plotW;const y=(row:Observation)=>top+plotH-((metric==='iron'?row.combat_iron_tonnes:row.soldiers_thousands)/ymax)*plotH;
    ...
    groups.forEach(([name,rows])=>{const color=colors[name]||'#66747d';ctx.strokeStyle=color;ctx.lineWidth=name==='Rome'?3.2:2;ctx.setLineDash(name==='Rome'?[]:[5,5]);if(rows.length>1){ctx.beginPath();rows.forEach((row,i)=>{const xx=x(row.year),yy=y(row);if(i===0)ctx.moveTo(xx,yy);else ctx.lineTo(xx,yy);});ctx.stroke();}ctx.setLineDash([]);rows.forEach(row=>{ctx.fillStyle=color;ctx.beginPath();ctx.arc(x(row.year),y(row),name==='Rome'?3.2:5,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.stroke();});});
  ...
  return <div className="capacity-chart">
    ...
    <div className="polity-legend">{Object.keys(colors).map(name=><span key={name}><i style={{background:colors[name]}} />{name}</span>)}</div>
