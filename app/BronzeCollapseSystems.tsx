...
export default function BronzeCollapseSystems(){
  ...
  return <div className="collapse-chart" style={{'--collapse-color':colors[active.case_id]} as React.CSSProperties}>
    <div className="collapse-summary"><div><span>Evidence windows</span><b>6</b><small>five cases + comparison</small></div><div><span>Shared collapse year</span><b>0</b><small>ranges remain ranges</small></div><div><span>Universal causes</span><b>0</b><small>stress is not sufficient cause</small></div></div>
    ...
    <section className="collapse-focus"><header><div><span>{active.region} · {label(active.evidence_kind)}</span><h5>{active.place}</h5></div><div><b>{active.anchor_value}</b><small>{active.anchor_label}</small></div></header><p className="collapse-reading">{active.interpretation}</p>
      ...
      <div className="collapse-limit"><div><span>What this does not prove</span><p>{active.limits}</p></div><div><span>Source keys</span><b>{active.source_keys}</b></div></div>
    </section>
    <div className="cradle-downloads collapse-downloads"><p><b>A pattern, not one event.</b> Dates orient unlike evidence. The page computes no cause, collapse, resilience, or population-loss score.</p><a href={dataUrl} download>Download chronology CSV ↓</a></div>
  </div>;
}
