...
export default function BronzeMetalNetwork(){
  ...
  return <div className="bronze-network-chart">
    ...
    <section className="bronze-network-readout" aria-live="polite"><div><h5>{selected.name}</h5><b>{selected.display_date} · {label(selected.material)}</b></div><div><p>{selected.observation}</p><small>{selected.limits}</small></div></section>
