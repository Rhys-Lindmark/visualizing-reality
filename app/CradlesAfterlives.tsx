...
export default function CradlesAfterlives(){
  ...
  if(state!=='ready'||!selected)return <div className={`data-state cradle-afterlives-data-state ${state}`} role={state==='error'?'alert':'status'}><b>{state==='error'?'The afterlives evidence did not load':'Loading historical pathways…'}</b><span>{state==='error'?error:'Comparing political breaks and visible afterlives'}</span>{state==='error'&&<button type="button" onClick={retry}>Retry evidence</button>}</div>;
  ...
  return <div className="afterlives-chart" style={{'--afterlife-color':colors[selected.region_slug]} as React.CSSProperties}>
    ...
    <section className="afterlives-readout"><div><span>What changed</span><h5>{selected.interpretation}</h5></div></section>
    <section className="afterlives-overview"><header><div><span>Six political breaks and their visible afterlives</span><h5>Capitals fell while people and institutions took new forms</h5></div></header><div>{rows.map(row=><button type="button" key={row.region_slug} onClick={()=>setSelectedSlug(row.region_slug)} className={row.region_slug===selected.region_slug?'active':''} style={{'--tab-color':colors[row.region_slug]} as React.CSSProperties}><i/><span>{row.region}</span><b>{row.pathway_label}</b><small>{row.afterlife}</small></button>)}</div></section>
  </div>;
}
